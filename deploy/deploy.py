...
from deploy.common.connect import connect_linux, run_ssh, connect_web3
from deploy.common.key import generate_key
from deploy.common.load_file import LoadFile, get_node_list
...
class BaseDeploy:
    def __init__(self, platon=conf.PLATON_BIN, cbft=conf.CBFT, keystore=conf.KEYSTORE, genesis=conf.GENESIS_TEMPLATE,
                 deploy_path=conf.DEPLOY_PATH, net_type=None, syncmode="full", config=conf.CONFIG_FILE):
        """
        部署节点
...
class AutoDeployPlaton(BaseDeploy):
    def __init__(self, platon=conf.PLATON_BIN, cbft=conf.CBFT, keystore=conf.KEYSTORE, genesis=conf.GENESIS_TEMPLATE,
                 deploy_path=conf.DEPLOY_PATH, net_type=None, syncmode="full", config=conf.CONFIG_FILE, sup_template=conf.SUP_TEMPLATE,
                 sup_tmp=conf.SUP_TMP,
                 ):
        super(AutoDeployPlaton, self).__init__(platon, cbft, keystore, genesis,
                                               deploy_path, net_type, syncmode, config)
...
if __name__ == "__main__":
    s = AutoDeployPlaton()
    s.deploy_default_yml(abspath("./deploy/node/7_node.yml"))
    # s.kill_of_yaml(abspath("./deploy/node/test_node.yml"))
